...
public class AuthController {
    ...
    @Operation(summary = "Authenticate user and return JWT token", description = "Authenticates a user with email and password. Returns a JWT token if successful.")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successful authentication, returns JWT token",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = LogInResultDto.class))),
    ...
    public LogInResultDto login(
            ...
            @Valid @RequestBody CredentialsDto creds) {
