...
@Tag(name = "Technician Management", description = "Endpoints for managing technicians. Only accessible by ADMIN users.")
@RestController
...
public class TechnicianManagementController {
    ...
    @Operation(summary = "Get a technician by ID", description = "Returns a technician by their unique identifier.")
    @ApiResponses({
    ...
    public TechnicianDto getOne(
            @Parameter(description = "UUID of the technician", required = true) @PathVariable UUID id) {
        return adminSvc.getTechnician(id);
    }

    @Operation(summary = "Create a new technician", description = "Creates a new technician with the provided details.")
    @ApiResponses({
    ...
    public TechnicianDto create(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                ...
                required = true,
                content = @Content(schema = @Schema(implementation = TechnicianCreateCommandDto.class))
            )
            @Valid @RequestBody TechnicianCreateCommandDto dto
    ) {
        return adminSvc.saveTechnician(dto);
    }

    @Operation(summary = "Update a technician", description = "Updates an existing technician by their ID.")
    @ApiResponses({
    ...
    public TechnicianDto update(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                ...
                required = true,
                content = @Content(schema = @Schema(implementation = TechnicianUpdateCommandDto.class))
            )
            @Valid @RequestBody TechnicianUpdateCommandDto dto
    ) {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Technician deleted",
