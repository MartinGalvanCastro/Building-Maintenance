...
@Tag(
        ...
        description =
                "Endpoints for managing residential complexes. Only accessible by ADMIN users.")
@RestController
...
public class ResidentialComplexController {
    ...
    @ApiResponses({
        @ApiResponse(
                ...
                content =
                        @Content(
                                mediaType = "application/json",
                                schema = @Schema(implementation = ResidentialComplexDto.class))),
        @ApiResponse(responseCode = "404", description = "Residential complex not found")
    })
    @GetMapping("/{id}")
    public ResidentialComplexDto getOne(
            @Parameter(description = "UUID of the residential complex", required = true)
                    @PathVariable
                    UUID id) {
    ...
    public ResidentialComplexDto create(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    @Valid
                    @RequestBody
                    ResidentialComplexCommandDto dto) {
