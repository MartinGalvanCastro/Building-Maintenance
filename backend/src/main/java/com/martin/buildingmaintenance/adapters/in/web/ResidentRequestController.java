...
@Tag(name = "Resident Maintenance Requests", description = "Endpoints for residents to manage their own maintenance requests.")
@RestController
...
public class ResidentRequestController {
    ...
    @Operation(summary = "List all maintenance requests for a resident", description = "Returns a list of all maintenance requests for the specified resident.")
    ...
    public List<MaintenanceRequestDto> list(
            @Parameter(description = "UUID of the resident", required = true) @PathVariable UUID residentId
    ) {
        return svc.listMyRequests(residentId);
    }

    @Operation(summary = "Get a maintenance request by ID for a resident", description = "Returns a maintenance request by its unique identifier for the specified resident.")
    @ApiResponses({
    ...
    @Operation(summary = "Create a new maintenance request for a resident", description = "Creates a new maintenance request for the specified resident.")
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "Maintenance request created",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = MaintenanceRequestDto.class))),
    ...
    @Operation(summary = "Update a maintenance request for a resident", description = "Updates an existing maintenance request for the specified resident.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Maintenance request updated",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = MaintenanceRequestDto.class))),
        @ApiResponse(responseCode = "400", description = "Invalid input data"),
        @ApiResponse(responseCode = "404", description = "Maintenance request not found")
    })
    ...
    @Operation(summary = "Cancel a maintenance request for a resident", description = "Cancels a maintenance request for the specified resident.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Maintenance request cancelled",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = CancelResponseDto.class))),
        @ApiResponse(responseCode = "404", description = "Maintenance request not found")
    })
