...
public class ResidentRequestServiceImpl implements ResidentRequestService {
    ...
    public List<MaintenanceRequestDto> listMyRequests(UUID residentId) {
        ...
        return requestRepo.findByResidentId(residentId).stream().map(mapper::toDto).toList();
    }
    ...
    public MaintenanceRequestDto getMyRequest(UUID residentId, UUID requestId) {
        ...
        var request =
                requestRepo
                        .findByIdAndResidentId(requestId, residentId)
    ...
    @Override
    public MaintenanceRequestDto updateMyRequest(
            UUID residentId, UUID requestId, UpdateRequestDto dto) {
    ...
    public CancelResponseDto cancelMyRequest(UUID residentId, UUID requestId) {
        ...
        var existing =
                requestRepo
                        .findByIdAndResidentId(requestId, residentId)
                        .orElseThrow(() -> new NotFoundException("Request not found"));
        var cancelled = existing.toBuilder().status(RequestStatus.CANCELLED).build();
        requestRepo.save(cancelled);
