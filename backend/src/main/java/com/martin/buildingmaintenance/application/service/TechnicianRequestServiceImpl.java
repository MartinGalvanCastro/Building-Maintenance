...
public class TechnicianRequestServiceImpl implements TechnicianRequestService {
    ...
    public List<MaintenanceRequestDto> listAssignments(UUID technicianId) {
        ...
        return repo.findByAssignedTechnicianId(technicianId).stream().map(mapper::toDto).toList();
    }

    @Override
    public MaintenanceRequestDto changeStatus(
            UUID technicianId, UUID requestId, ChangeStatusDto dto) {
        log.info(
                "Technician {} changing status of request {} to {}",
                technicianId,
                requestId,
                dto.status());
        ...
        MaintenanceRequest updated = existing.toBuilder().status(dto.status()).build();

        MaintenanceRequest saved = repo.save(updated);
        log.info(
                "Technician {} changed status of request {} to {}",
                technicianId,
                requestId,
                dto.status());
