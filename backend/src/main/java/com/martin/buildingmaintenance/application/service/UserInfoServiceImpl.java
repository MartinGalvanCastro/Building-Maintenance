...
public class UserInfoServiceImpl implements UserInfoService {
    ...
    public UserInfoDto getCurrentUserInfo(String authHeader) {
        ...
        if ("RESIDENT".equalsIgnoreCase(role)) {
            ...
            return resident.map(r -> new UserInfoDto(
                    r.getId().toString(),
                    r.getFullName(),
                    r.getEmail(),
                    r.getRole(),
                    residentialComplexMapper.toDto(r.getResidentialComplex()),
                    maintenanceRequestRepository.findByResidentId(r.getId()).stream()
            ...
            return admin.map(a -> new UserInfoDto(
                    a.getId().toString(),
                    a.getFullName(),
                    a.getEmail(),
                    a.getRole(),
            ...
            return tech.map(t -> new UserInfoDto(
                    t.getId().toString(),
                    t.getFullName(),
                    t.getEmail(),
                    t.getRole(),
                    null,
                    maintenanceRequestRepository.findByAssignedTechnicianId(t.getId()).stream()
                        .map(maintenanceRequestMapper::toSummary)
                        .toList()
